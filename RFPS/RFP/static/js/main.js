        ...
        success : function(data) {
        	...
        	if(data.userclass == 'PREPARER'){
        		if (data['status.desc'] == 'WORK'){
	            	...
	            	//replace Request button text as Update
	            	$('#btnRequest').show();
	    			$("#btnRequest").html('<span class="glyphicon glyphicon-edit"></span> Update');
	    			$('#btnRequest').attr('title','Update RFP');
        			//replace savepost text as update & post
	    			$('#btnSavePost').show();
	    			$('#btnSavePost').html('<span class="glyphicon glyphicon-check"></span> <span id="btn-label">Update & Post</span>');
	    			$('#btnSavePost').attr('title','Update & Post Request');
...
$("body").on("keyup",".override", function(event) {
	...
	var ewtamt = $('#wtaxamt' + entryno).val().replace(/,/g , "");
	if (ewtamt === ''){
		ewtamt = 0;
	}
	...
	if (grossamt !== ''){
		WTaxamt = parseFloat(ewtamt);
		
		NETamt = parseFloat(grossamt) - WTaxamt;
...
function rfpreadonly(){
	...
	$('.actbtns').attr('disabled',false);
	$('button.ui-widget').attr('disabled',false);
}
////////////////////////////////////////////////////////////////////////
//reset rfp elements
function rfpreset(){
...
function update_dept(){
	$.ajax({
        ...
        success : function(data) {
            ...
            }).keydown(function (e) {//disable tab
            	if (e.keyCode === 9)
            	e.preventDefault();
            });
